...
struct acui_information RealLive_cui_information = {
	...
	_T(".g00 .nwk .txt .ovk .cgm .dat .nwk"),	/* package */
	_T("0.7.5"),			/* revision */
	_T("痴漢公賊"),			/* author */
	_T("2009-3-26 20:03"),	/* date */
	NULL,					/* notion */
...
{
	g00_header_t g00_header;
	if (pkg->pio->readvec(pkg, &g00_header, 0, IO_SEEK_SET))
		return -CUI_EREADVEC;
...
/* 封包索引项解析函数 */
static int RealLive_g00_2_dir_parse_resource_info(struct package *pkg,
												  struct package_resource *pkg_res)
{
	g02_info_t_t *g02_info;

	g02_info = (g02_info_t *)pkg_res->actual_index_entry;
	sprintf(pkg_res->name, "%04d", pkg_res->index_number);
	pkg_res->name_length = -1;			/* -1表示名称以NULL结尾 */
	pkg_res->raw_data_length = g02_info->;
	pkg_res->actual_data_length = 0;
	pkg_res->offset = gen_entry->offset;
...
static int RealLive_g02_extract_resource(struct package *pkg,
										 struct package_resource *pkg_res)
{
	int reverse;
	DWORD g02p_size;
